...
- **A version gate that checks only `collection.json` is not a version gate.** (core 3.22.5 / C.1.3.5, 2026-07-01.) A hand-rolled build-and-prep script asserted `collection.json` was at the target but never checked the `api/*-manifest.json` `collection_version` fields — so the bump shipped with all 20 manifests a version behind. It was caught only by the push-time preflight (Check 2), waved past, and required a hotfix commit + a force-moved tag (safe only because nothing had consumed it yet). The manifest-alignment assertion + the manifest restamp belong in the **earliest** phase (the build-and-prep script's version gate — see the `release` task Step 3A/3C), not deferred to preflight. Prefer generating release scripts with the `release` task, which bakes both in, over hand-copying an older `push-*.sh`.
