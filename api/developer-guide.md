...
**"Should this workflow step be a script, and how far do I take it?"** (C.1.5.0 tier principle) -- Mechanical work sits at one of three maturity levels; push down to the lowest sustainable one. **Level 1 -- inline reasoning in the task:** the agent re-derives the same mechanical logic in-context every run (max tokens, max variance). **Level 2 -- the agent generates a script from a template each run:** better, but the agent still transcribes the logic every run, so it costs output tokens proportional to script size AND can silently drop logic (exactly how bug `clonescripttagassumption` slipped through -- the template was correct, deployed, and referenced, yet the agent hand-wrote a script that omitted the branch-HEAD case). **Level 3 -- a committed, parameterized script the agent invokes with DATA only:** logic written and tested once and version-controlled; the agent emits a tiny data payload (a manifest / ops-array), not control flow. Rule: **push Level 2 -> Level 3 whenever the script logic is stable across runs.** Level 2 is only justified when the body genuinely varies per run in ways that cannot be parameterized. C.1.5.0 reference implementations: core `lib/clone/` (clone tooling + data manifest), core `lib/permission-spec/build-permission-spec.js` (ops-array in, validated spec out), and this collection`s `lib/release/` (release-delta manifest + committed prep/push). The `optimize` task flags Level-1 and Level-2 patterns as promotion candidates.

**"How should adapter tests be written so preflight trusts them?"** (C.1.5.0, closes the `getpermsinvalidownerfield` regression class) -- Adapter unit tests MUST exercise the **real API field contract**, not a permissive mock that ignores the fields the code requests. That regression shipped because the `permissions.list` mock ignored the `fields` selector, so an invalid `fields` string passed tests and only broke on-org. Assert the exact field/query strings the adapter sends, or use a mock that rejects unknown fields. Preflight Check 14 verifies the built bundle checksum + parseability, but it cannot tell whether a mock is honest -- that is on the test.
